...
class ChQuotaHandler(ObjectHandler):
    ...
    def emit(self, op: Op, db_name: Optional[str] = None, **kwargs: Any) -> List[MigrationStatement]:
        ...
        if op.object_type == "create_ch_quota":
            interval = op.upgrade_attrs.get("interval", "")
            limits = op.upgrade_attrs.get("limits", {})
            ...
            stmts.append(MigrationStatement(
                ...
                upgrade_sql=f"CREATE QUOTA IF NOT EXISTS {name} FOR INTERVAL {interval} {limit_clauses}{roles_clause};",
                rollback_sql=f"DROP QUOTA IF EXISTS {name};",
            ))
        elif op.object_type == "drop_ch_quota":
            stmts.append(MigrationStatement(
                order=self.statement_order,
                upgrade_sql=f"DROP QUOTA IF EXISTS {name};",
